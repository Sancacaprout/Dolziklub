...
export function AlbumCard({
  ...
  const cover = coverSrc ? <Image src={coverSrc} alt={`Pochette de ${album.title} par ${album.artist}`} fill quality={90} sizes={list ? "170px" : compact ? "110px" : "(max-width: 700px) calc(100vw - 2rem), (max-width: 1100px) 48vw, 460px"} onError={() => { if (coverSrc !== fallbackCover) setFailedCoverKey(preferredCoverKey); }} /> : <Image src={fallbackCover} alt={`Pochette non disponible pour ${album.title} par ${album.artist}`} fill quality={90} sizes={list ? "170px" : compact ? "110px" : "(max-width: 700px) calc(100vw - 2rem), (max-width: 1100px) 48vw, 460px"} />;
