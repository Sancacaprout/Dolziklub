...
type DrawEntry = { id: string; draw_number: number; position: number; proposed_by: string | null; listened_by: string | null; proposed_by_name: string | null; listened_by_name: string | null; album_title: string | null; album_artist: string | null; cover_path: string | null; youtube_music_url?: string | null };
type DrawMeta = { draw_number: number; participant_usernames: string[]; status: "draft" | "published" | "locked"; avoid_repeated_pairs: boolean };
type ReviewRecord = { id?: string; album_id: string; review: string; rating: number; best_track: string | null; worst_track: string | null; best_track_youtube_music_url?: string | null; best_track_youtube_url?: string | null; worst_track_youtube_music_url?: string | null; worst_track_youtube_url?: string | null };
...
const ratingChoices = Array.from({ length: 6 }, (_, index) => index);
const quizQuestions = [
...
function HistoricalPendingReviews({ albums, member }: { albums: Album[]; member: SignedMember | null }) {
  ...
  const save = async ({ entryId, review, rating, bestTrack, worstTrack }: ReviewPayload) => {
    if (!configured || !review.trim() || !Number.isInteger(rating) || rating < 0 || rating > 5) return;
...
export function TableurBoard({ albums }: { albums: Album[] }) {
  ...
  const loadEntries = useCallback(async () => { if (!configured) return; const columns = member ? "id, draw_number, position, proposed_by, listened_by, proposed_by_name, listened_by_name, album_title, album_artist, cover_path, youtube_music_url" : "id, draw_number, position, proposed_by_name, listened_by_name, album_title, album_artist, cover_path, youtube_music_url"; const { data, error } = await getSupabaseBrowserClient().from("club_draw_entries").select(columns).order("draw_number", { ascending: true }).order("position", { ascending: true }); if (!error) setEntries((data ?? []) as DrawEntry[]); }, [configured, member]);
  ...
  const saveReview = async ({ entryId, review, rating, bestTrack, worstTrack, bestMatch, worstMatch }: ReviewPayload) => { if (!configured || !member || !review.trim() || !Number.isInteger(rating) || rating < 0 || rating > 5) return; setSavingId(entryId); setMessage(""); try { const { data: saved, error } = await getSupabaseBrowserClient().from("member_album_reviews").upsert({ album_id: entryId, member_id: member.id, review: review.trim(), rating, best_track: bestTrack.trim() || null, worst_track: worstTrack.trim() || null }, { onConflict: "album_id,member_id" }).select("id").single(); if (error || !saved?.id) throw error ?? new Error("Verdict introuvable."); const { data: session } = await getSupabaseBrowserClient().auth.getSession(); const payload = (selectionType: "best" | "worst", title: string, match: import("@/lib/music-matching").MusicCandidate | null | undefined) => ({ selectionType, title: title.trim(), artists: match?.artist ? [match.artist] : [], youtubeVideoId: match?.resourceType === "video" ? match.resourceId : null, youtubeMusicUrl: match?.youtubeMusicUrl ?? null, youtubeUrl: match?.youtubeUrl ?? null, thumbnailUrl: match?.thumbnailUrl ?? null, source: match ? "youtube_search" : "manual", verified: false }); const tracks = await fetch("/api/music/track-selections", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${session.session?.access_token ?? ""}` }, body: JSON.stringify({ entryId, reviewId: saved.id, selections: [payload("best", bestTrack, bestMatch), payload("worst", worstTrack, worstMatch)] }) }); if (!tracks.ok) throw new Error("Le verdict est enregistré, mais les liens des morceaux n’ont pas pu être sauvegardés."); setMessage("Verdict et liens de morceaux enregistrés dans le tirage."); await Promise.all([loadReviews(), loadPublicReviews()]); setFocusedProposal(null); setFocusedReview(null); setActiveTab("archive"); } catch (error) { setMessage(error instanceof Error ? error.message : "Ce verdict n’a pas pu être enregistré."); } finally { setSavingId(null); } };
