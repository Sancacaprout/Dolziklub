...
function ProposalCard({ entry, saving, onSave, onDelete }: { entry: DrawEntry; saving: boolean; onSave: (payload: ProposalPayload) => void; onDelete: (entryId: string) => void }) {
...
export function TableurBoard({ albums }: { albums: Album[] }) {
  const configured = isSupabaseConfigured(); const [activeTab, setActiveTab] = useState<Tab>("archive"); const [member, setMember] = useState<SignedMember | null>(null); const [isAdmin, setIsAdmin] = useState(false); const [entries, setEntries] = useState<DrawEntry[]>([]); const [draws, setDraws] = useState<DrawMeta[]>([]); const [reviews, setReviews] = useState<ReviewRecord[]>([]); const [kouizeProfiles, setKouizeProfiles] = useState<PublicKouizeProfile[]>([]); const [savingId, setSavingId] = useState<string | null>(null); const [message, setMessage] = useState(""); const [focusedProposal, setFocusedProposal] = useState<string | null>(null); const [focusedReview, setFocusedReview] = useState<string | null>(null);
  ...
  useEffect(() => { const timer = setTimeout(() => { void loadEntries(); void loadDraws(); void syncAccess(); if (configured) void getSupabaseBrowserClient().from("member_public_profiles").select("username, kouize").then((result: { data: unknown }) => setKouizeProfiles((result.data ?? []) as PublicKouizeProfile[])); }, 0); return () => clearTimeout(timer); }, [configured, loadDraws, loadEntries, syncAccess]);
  ...
  const deleteProposal = async (entryId: string) => { if (!configured || !member) return; setSavingId(entryId); setMessage(""); const { error } = await getSupabaseBrowserClient().from("club_draw_entries").update({ album_title: null, album_artist: null, cover_path: null }).eq("id", entryId); setSavingId(null); if (error) setMessage("Cet album ne peut plus être retiré : un verdict a peut-être déjà été enregistré."); else { setMessage("Album retiré de ta proposition."); await loadEntries(); } };
