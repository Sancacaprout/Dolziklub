...
import { matchOrientedDraw, type OrientedDrawResult } from "@/lib/oriented-draw-matcher";
...
type PairingSummary = { repeatedPairCount?: number; warning?: string | null; repeatedPairs?: Array<{ proposer: string; listener: string; previousDraws: number[] }> };
type DrawMeta = { draw_number: number; participant_usernames: string[]; status: "draft" | "published" | "locked"; avoid_repeated_pairs: boolean; draw_type: "standard" | "global"; global_proposer_username: string | null; created_at: string; pairing_summary?: PairingSummary | null };
...
function AdminDraws({ albums = archivedAlbums, entries, draws, savingId, onCreate, onRegenerate, onDelete, onValidate, onPublish }: { albums?: Album[]; entries: DrawEntry[]; draws: DrawMeta[]; savingId: string | null; onCreate: (config: { participants: string[]; drawType: "standard" | "global"; globalProposer: string | null }) => void; onRegenerate: (draw: DrawMeta) => void; onDelete: (draw: number) => void; onValidate: (draw: number, assignments: Array<{ entry: DrawEntry; proposer: string; listener: string }>) => Promise<void>; onPublish: (draw: number) => void }) {
  ...
  const validateDraw = async (draw: DrawMeta) => {
    ...
    const invalidIds = [...new Set([...incomplete, ...selfPairs, ...duplicateProposers, ...duplicateListeners])];
    if (invalidIds.length) {
      const reasons = [incomplete.length ? "des lignes incomplètes" : "", selfPairs.length ? "des membres attribués à eux-mêmes" : "", duplicateProposers.length ? "des proposeurs en double" : "", duplicateListeners.length ? "des auditeurs en double" : ""].filter(Boolean).join(", ");
      setDuoNotice({ draw: draw.draw_number, invalidIds, message: "Alerte : corrige " + reasons + " avant de publier." });
      return;
    }
    setDuoNotice({ draw: draw.draw_number, invalidIds: [], message: repeatedPairs.length ? `${repeatedPairs.length} relation${repeatedPairs.length > 1 ? "s" : ""} historique${repeatedPairs.length > 1 ? "s" : ""} nécessaire${repeatedPairs.length > 1 ? "s" : ""} : le tirage reste cohérent.` : "Tous les duos sont valides et orientés de façon inédite." });
  ...
  return <section className="draw-sheet admin-draw-sheet">
    <div className="draw-creator draw-creator--global-ready">
      <div><span className="eyebrow">ADMINISTRATION</span><h3>Préparer un nouveau tirage</h3><p>{drawType === "global" ? "Un membre choisit l’album commun. Tous les autres rendent chacun leur propre verdict." : "Chaque membre reçoit une ligne « proposé par » et une ligne « écouté par ». Tout l’historique consolidé est analysé dans le sens proposeur → auditeur."}</p></div>
    ...
    {draws.map((draw) => {
      ...
      return <section className={"draw-section" + (global ? " draw-section--global" : "")} key={draw.draw_number}>
        <div className="draw-heading"><span className="eyebrow">TIRAGE {String(draw.draw_number).padStart(2, "0")} · {draw.status === "locked" ? "VERROUILLÉ" : draw.status === "published" ? "PUBLIÉ" : "BROUILLON"} · {global ? "ÉCOUTE GLOBALE" : "DUOS UNIQUES"}</span>{draw.status !== "locked" && <div>{!global && draw.status === "draft" && <button className="sheet-entry-action" type="button" disabled={savingId === "regenerate-" + draw.draw_number} onClick={() => onRegenerate(draw)}>{savingId === "regenerate-" + draw.draw_number ? "Recherche…" : "Régénérer les duos"}</button>}{!global && <button className="sheet-entry-action" type="button" disabled={savingId === "validate-" + draw.draw_number} onClick={() => void validateDraw(draw)}>{savingId === "validate-" + draw.draw_number ? "Validation…" : "Valider les duos"}</button>}<button className="sheet-entry-action" type="button" disabled={draw.status !== "draft" || !complete || savingId === "publish-" + draw.draw_number} onClick={() => onPublish(draw.draw_number)}>{savingId === "publish-" + draw.draw_number ? "Publication…" : "Publier ce tirage"}</button><button className="sheet-entry-action sheet-entry-action--delete" type="button" disabled={savingId === "delete-" + draw.draw_number} onClick={() => onDelete(draw.draw_number)}>Supprimer</button></div>}</div>
        {global && <p className="global-draw-summary"><b>{memberName(draw.global_proposer_username)}</b> choisit un album unique · {rows.length} membre{rows.length > 1 ? "s" : ""} devront l’écouter.</p>}{!global && draw.pairing_summary?.warning && <p className="duo-notice duo-notice--alert">{draw.pairing_summary.warning}</p>}
...
export function TableurBoard({ albums }: { albums: Album[] }) {
  ...
  const persistDraw = async ({ participants, drawType, globalProposer }: { participants: string[]; drawType: "standard" | "global"; globalProposer: string | null }) => {
    if (!configured || !isAdmin) return false;
    setSavingId("new-draw");
    setMessage(drawType === "global" ? "Création de l’écoute globale…" : "Analyse de l’historique des duos…");
    try {
      if (drawType === "global") {
        const { error } = await getSupabaseBrowserClient().rpc("admin_create_club_draw", {
          p_participant_usernames: participants,
          p_avoid_repeated_pairs: false,
          p_draw_type: drawType,
          p_global_proposer_username: globalProposer,
          p_legacy_forbidden_pairs: [],
        });
        if (error) throw error;
        setMessage("Écoute globale créée : un verdict indépendant est attendu pour chaque auditeur.");
      } else {
        const result = await prepareClassicAssignments(participants, null, Date.now());
        setMessage("Recherche de nouvelles associations…");
        const { error } = await getSupabaseBrowserClient().rpc("admin_create_classic_draw_from_assignments", {
          p_proposer_ids: result.assignments.map((assignment) => assignment.proposer.id),
          p_listener_ids: result.assignments.map((assignment) => assignment.listener.id),
          p_pairing_summary: pairingSummary(result),
        });
        if (error) throw error;
        setMessage(result.warning ?? "Vérification terminée : toutes les relations orientées sont inédites.");
      }
      await Promise.all([loadEntries(), loadDraws()]);
      return true;
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Le tirage n’a pas pu être créé.");
      return false;
    } finally {
      setSavingId(null);
    }
  };
  const regenerateDraw = async (draw: DrawMeta) => {
    if (!configured || !isAdmin || draw.status !== "draft" || draw.draw_type !== "standard") return;
    setSavingId(`regenerate-${draw.draw_number}`);
    setMessage("Analyse de l’historique des duos…");
    try {
      const result = await prepareClassicAssignments(draw.participant_usernames, draw.draw_number, Date.now());
      setMessage("Vérification du tirage…");
      const { error } = await getSupabaseBrowserClient().rpc("admin_replace_classic_draw_assignments", {
        p_draw_number: draw.draw_number,
        p_proposer_ids: result.assignments.map((assignment) => assignment.proposer.id),
        p_listener_ids: result.assignments.map((assignment) => assignment.listener.id),
        p_pairing_summary: pairingSummary(result),
      });
      if (error) throw error;
      setMessage(result.warning ?? "Nouvelles associations générées : toutes les relations sont inédites.");
      await Promise.all([loadEntries(), loadDraws()]);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Les duos n’ont pas pu être régénérés.");
    } finally {
      setSavingId(null);
    }
  };
  ...
  const deleteExtraListening = async (requestId: string) => {
    if (!configured || !isAdmin || !confirm("Supprimer définitivement cette écoute supplémentaire ?")) return;
    setSavingId(`delete-extra-${requestId}`);
    setMessage("");
    const { error } = await getSupabaseBrowserClient().rpc("admin_delete_extra_listening_request", {
    ...
    setMessage("Écoute supplémentaire supprimée.");
  ...
  const openExtraWorkspace = (requestId: string | null = null) => {
    setExtraFocusId(requestId);
    setExtraScrollRequest((current) => current + 1);
    setActiveTab("selection");
  };
  const openExtraRequest = (requestId: string) => openExtraWorkspace(requestId);
