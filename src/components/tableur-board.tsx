...
const ReviewCard = ({ entry, existing, saving, onSave, onReset }: { entry: DrawEntry; existing?: ReviewRecord; saving: boolean; onSave: (payload: ReviewPayload) => void; onReset: (entryId: string) => void }) => <ReviewAssistantCard entry={entry} existing={existing} coverUrl={coverUrl(entry.cover_path) ?? archiveForEntry(entry)?.cover ?? null} saving={saving} onSave={onSave} onReset={onReset} />;
