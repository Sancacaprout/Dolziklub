...
type DrawMeta = { draw_number: number; participant_usernames: string[]; status: "draft" | "published" | "locked" };
...
const quizQuestions = [
  ...
  { key: "curiosity", label: "DÉTOUR", question: "Le territoire musical qui t’intrigue", prompt: "Le genre ou l’univers que tu veux mieux explorer." },
...
function LiveDraws({ entries, member, onOpenProposal, onOpenReview }: { entries: DrawEntry[]; member: SignedMember | null; onOpenProposal: (id: string) => void; onOpenReview: (id: string) => void }) {
  // The active rows are handled from the dedicated Sélection workspace.
  // Keeping them out of the archive avoids duplicating a trailing draw table.
  return null;
  const draws = new Map<number, DrawEntry[]>();
  entries.forEach((entry) => draws.set(entry.draw_number, [...(draws.get(entry.draw_number) ?? []), entry]));
  return <>{[...draws.entries()].map(([drawNumber, rows]) => <section className="draw-section draw-section--live" key={drawNumber}><div className="draw-heading"><span className="eyebrow">TIRAGE {String(drawNumber).padStart(2, "0")}</span><span>{rows.length} emplacement{rows.length > 1 ? "s" : ""} en cours</span></div><div className="sheet-scroll"><table className="sheet-table"><thead><tr><th>Album · Artiste</th><th>Proposé par</th><th>Écouté par</th><th>État</th></tr></thead><tbody>{rows.sort((a, b) => a.position - b.position).map((entry) => { const canPropose = member ? isAssignedToMember(entry, member, "proposer") && isEmptyAlbumSlot(entry) : false; const canReview = member ? isAssignedToMember(entry, member, "listener") && !isEmptyAlbumSlot(entry) : false; return <tr className={(canPropose || canReview) ? "sheet-row--action" : ""} key={entry.id} onClick={() => canPropose ? onOpenProposal(entry.id) : canReview ? onOpenReview(entry.id) : undefined}><td>{entry.album_title ? <span className="sheet-album-link"><b>{entry.album_title}</b><span>{entry.album_artist}</span></span> : <span className="sheet-placeholder-album">Album – Artiste</span>}</td><td><span className="sheet-member">{memberName(entry.proposed_by_name)}</span></td><td><span className="sheet-member">{memberName(entry.listened_by_name)}</span></td><td>{canPropose ? <button className="sheet-entry-action" onClick={(event) => { event.stopPropagation(); onOpenProposal(entry.id); }}>Proposer l’album</button> : canReview ? <button className="sheet-entry-action" onClick={(event) => { event.stopPropagation(); onOpenReview(entry.id); }}>Rendre l’écoute</button> : entry.album_title ? <span className="sheet-pending">À écouter</span> : <span className="sheet-pending">En attente</span>}</td></tr>; })}</tbody></table></div></section>)}</>;
}

function ProposalCard({ entry, saving, onSave }: { entry: DrawEntry; saving: boolean; onSave: (payload: ProposalPayload) => void }) {
...
function SelectionWorkspace({ entries, currentDraw, member, reviews, focusedProposal, focusedReview, savingId, onProposal, onReview }: { entries: DrawEntry[]; currentDraw: number | null; member: SignedMember | null; reviews: ReviewRecord[]; focusedProposal: string | null; focusedReview: string | null; savingId: string | null; onProposal: (payload: ProposalPayload) => void; onReview: (payload: ReviewPayload) => void }) {
  const currentEntries = currentDraw ? entries.filter((entry) => entry.draw_number === currentDraw) : [];
...
function AdminDraws({ entries, draws, savingId, onCreate, onDelete, onAssign, onPublish }: { entries: DrawEntry[]; draws: DrawMeta[]; savingId: string | null; onCreate: (participants: string[]) => void; onDelete: (draw: number) => void; onAssign: (entry: DrawEntry, proposer: string, listener: string, confirmReset: boolean) => void; onPublish: (draw: number) => void }) {
  ...
  return <section className="draw-sheet admin-draw-sheet"><div className="draw-creator"><div><span className="eyebrow">ADMINISTRATION</span><h3>Préparer un nouveau tirage</h3><p>Choisis les membres présents : chacun recevra exactement une ligne « proposé par » et une ligne « écouté par ».</p><div className="draw-participants">{roster.map((member) => <label key={member.username}><input type="checkbox" checked={participants.includes(member.username)} onChange={() => toggleParticipant(member.username)} /> {member.displayName}</label>)}</div></div><button type="button" className="button" disabled={savingId === "new-draw" || participants.length < 2} onClick={() => onCreate(participants)}>{savingId === "new-draw" ? "Création…" : `Créer un tirage de ${participants.length} membres`}</button></div>{draws.map((draw) => { const rows = (rowsByDraw.get(draw.draw_number) ?? []).sort((a, b) => a.position - b.position); const editable = draw.status !== "locked"; const options = roster.filter((member) => draw.participant_usernames.includes(member.username)); const complete = rows.length === draw.participant_usernames.length && rows.every((entry) => { const current = field(entry); return Boolean(current.proposer && current.listener); }); return <section className="draw-section" key={draw.draw_number}><div className="draw-heading"><span className="eyebrow">TIRAGE {String(draw.draw_number).padStart(2, "0")} · {draw.status === "locked" ? "VERROUILLÉ" : draw.status === "published" ? "PUBLIÉ" : "BROUILLON"}</span>{editable && <div><button className="sheet-entry-action" type="button" disabled={draw.status !== "draft" || !complete || savingId === `publish-${draw.draw_number}`} onClick={() => onPublish(draw.draw_number)}>{savingId === `publish-${draw.draw_number}` ? "Publication…" : "Publier ce tirage"}</button><button className="sheet-entry-action sheet-entry-action--delete" type="button" disabled={savingId === `delete-${draw.draw_number}`} onClick={() => onDelete(draw.draw_number)}>Supprimer</button></div>}</div><div className="sheet-scroll"><table className="sheet-table admin-draw-table"><thead><tr><th>Ligne</th><th>Proposé par</th><th>Écouté par</th><th>Action</th></tr></thead><tbody>{rows.map((entry) => { const current = field(entry); const proposedOptions = options.filter((member) => member.username === current.proposer || !rows.some((other) => other.id !== entry.id && field(other).proposer === member.username)); const listenerOptions = options.filter((member) => member.username === current.listener || !rows.some((other) => other.id !== entry.id && field(other).listener === member.username)); const changed = current.proposer !== (entry.proposed_by_name ?? "") || current.listener !== (entry.listened_by_name ?? ""); return <tr key={entry.id}><td>{String(entry.position).padStart(2, "0")}</td><td><MemberSelect value={current.proposer} options={proposedOptions} disabled={!editable} label="Choisir le proposeur" onChange={(next) => change(entry, "proposer", next)} /></td><td><MemberSelect value={current.listener} options={listenerOptions} disabled={!editable} label="Choisir la personne qui écoute" onChange={(next) => change(entry, "listener", next)} /></td><td>{editable && <button type="button" className="sheet-entry-action" disabled={!changed || savingId === entry.id} onClick={() => { const hasContent = Boolean(entry.album_title || entry.album_artist); if (hasContent && !confirm("Changer cette attribution supprimera l’album et son avis. Continuer ?")) return; onAssign(entry, current.proposer, current.listener, hasContent); }}>{savingId === entry.id ? "…" : "Enregistrer"}</button>}</td></tr>; })}</tbody></table></div>{draw.status === "draft" && !complete && <p className="sheet-pending">Attribue chaque participant une fois dans chaque colonne avant de publier.</p>}</section>; })}</section>;
}

function Kouize({ answers, member, isAdmin }: { answers: KouizeAnswer[]; member: SignedMember | null; isAdmin: boolean }) {
  const answerMap = new Map(answers.map((answer) => [`${answer.member_username}:${answer.question_key}`, answer.answer]));
  return <section className="quiz-sheet"><div className="quiz-intro kouize-intro"><p className="eyebrow">KOUIZE</p><h2>Un peu plus de goût,<br /><em>un peu moins de hasard.</em></h2><p>{member && !isAdmin ? "Modifie ton Kouize depuis Mon compte." : "Les réponses du club aident à mieux viser les prochaines propositions."}</p></div><div className="quiz-scroll kouize-scroll"><table className="quiz-table kouize-table"><thead><tr><th>Repère</th>{quizMembers.map((name) => <th key={name}>{name}</th>)}</tr></thead><tbody>{quizQuestions.map((question) => <tr key={question.key}><th><span>{question.label}</span><b>{question.question}</b><small>{question.prompt}</small></th>{quizMembers.map((name) => { const username = roster.find((memberEntry) => memberEntry.displayName === name)?.username ?? name.toLowerCase(); return <td key={name}><span className="kouize-answer">{answerMap.get(`${username}:${question.key}`) ?? "—"}</span></td>; })}</tr>)}</tbody></table></div></section>;
}

export function TableurBoard({ albums }: { albums: Album[] }) {
  const configured = isSupabaseConfigured(); const [activeTab, setActiveTab] = useState<Tab>("archive"); const [member, setMember] = useState<SignedMember | null>(null); const [isAdmin, setIsAdmin] = useState(false); const [entries, setEntries] = useState<DrawEntry[]>([]); const [draws, setDraws] = useState<DrawMeta[]>([]); const [reviews, setReviews] = useState<ReviewRecord[]>([]); const [kouizeAnswers, setKouizeAnswers] = useState<KouizeAnswer[]>([]); const [savingId, setSavingId] = useState<string | null>(null); const [message, setMessage] = useState(""); const [focusedProposal, setFocusedProposal] = useState<string | null>(null); const [focusedReview, setFocusedReview] = useState<string | null>(null);
  const visibleTabs = isAdmin ? [...tabs, adminTab] : tabs; const active = visibleTabs.find((tab) => tab.id === activeTab) ?? tabs[0];
  const loadEntries = useCallback(async () => { if (!configured) return; const columns = member ? "id, draw_number, position, proposed_by, listened_by, proposed_by_name, listened_by_name, album_title, album_artist, cover_path" : "id, draw_number, position, proposed_by_name, listened_by_name, album_title, album_artist, cover_path"; const { data, error } = await getSupabaseBrowserClient().from("club_draw_entries").select(columns).order("draw_number", { ascending: true }).order("position", { ascending: true }); if (!error) setEntries((data ?? []) as DrawEntry[]); }, [configured, member]);
  const loadDraws = useCallback(async () => { if (!configured) return; const { data, error } = await getSupabaseBrowserClient().from("club_draws").select("draw_number, participant_usernames, status").order("draw_number", { ascending: true }); if (!error) setDraws((data ?? []) as DrawMeta[]); }, [configured]);
  const loadReviews = useCallback(async () => { if (!configured) return; const { data, error } = await getSupabaseBrowserClient().from("member_album_reviews").select("album_id, review, rating, best_track, worst_track"); if (!error) setReviews((data ?? []) as ReviewRecord[]); }, [configured]);
  ...
  useEffect(() => { const timer = setTimeout(() => { void loadEntries(); void loadDraws(); void syncAccess(); if (configured) void getSupabaseBrowserClient().from("member_kouize_answers").select("member_username, question_key, answer").then((result: { data: unknown }) => setKouizeAnswers((result.data ?? []) as KouizeAnswer[])); }, 0); return () => clearTimeout(timer); }, [configured, loadDraws, loadEntries, syncAccess]);
  ...
  const saveProposal = async ({ entryId, title, artist, file }: ProposalPayload) => { if (!configured || !member) return; setSavingId(entryId); setMessage(""); let cover_path: string | undefined; try { if (file) { if (!new Set(["image/jpeg", "image/png", "image/webp"]).has(file.type) || file.size > 5 * 1024 * 1024) throw new Error("Choisis une image JPG, PNG ou WebP de 5 Mo maximum."); const extension = file.name.split(".").pop()?.toLowerCase() || "jpg"; const path = `${member.id}/${entryId}.${extension}`; const { error } = await getSupabaseBrowserClient().storage.from("album-covers").upload(path, file, { upsert: true, contentType: file.type, cacheControl: "31536000" }); if (error) throw error; cover_path = path; } const update = { album_title: title.trim(), album_artist: artist.trim(), ...(cover_path ? { cover_path } : {}) }; const { error } = await getSupabaseBrowserClient().from("club_draw_entries").update(update).eq("id", entryId); if (error) throw error; setMessage("Album proposé : il apparaît maintenant dans le tirage."); await loadEntries(); } catch (error) { setMessage(error instanceof Error ? error.message : "L’album n’a pas pu être enregistré."); } finally { setSavingId(null); } };
  const saveReview = async ({ entryId, review, rating, bestTrack, worstTrack }: ReviewPayload) => { if (!configured || !member || !review.trim() || !Number.isFinite(rating)) return; setSavingId(entryId); setMessage(""); const { error } = await getSupabaseBrowserClient().from("member_album_reviews").upsert({ album_id: entryId, member_id: member.id, review: review.trim(), rating, best_track: bestTrack.trim() || null, worst_track: worstTrack.trim() || null }, { onConflict: "album_id,member_id" }); setSavingId(null); if (error) setMessage("Ce verdict n’a pas pu être enregistré."); else { setMessage("Verdict enregistré."); await loadReviews(); } };
  const createDraw = async (participants: string[]) => { if (!configured || !isAdmin) return; setSavingId("new-draw"); setMessage(""); const { error } = await getSupabaseBrowserClient().rpc("admin_create_club_draw", { p_participant_usernames: participants }); setSavingId(null); if (error) setMessage(error.message); else { setMessage("Nouveau brouillon créé."); await Promise.all([loadEntries(), loadDraws()]); } };
  const assignEntry = async (entry: DrawEntry, proposer: string, listener: string, confirmReset: boolean) => { if (!configured || !isAdmin) return; setSavingId(entry.id); setMessage(""); const { error } = await getSupabaseBrowserClient().rpc("admin_update_club_draw_entry", { p_entry_id: entry.id, p_proposer_username: proposer, p_listener_username: listener, p_confirm_reset: confirmReset }); setSavingId(null); if (error) setMessage(error.message); else { setMessage("Attribution enregistrée."); await loadEntries(); } };
  ...
  const currentPublishedDraw = draws.filter((draw) => draw.status === "published").reduce((latest, draw) => Math.max(latest, draw.draw_number), 0) || null;
  return <section className="tableur-board" aria-label="Tableur du DOL ZIKLUB"><div className="sheet-tabs" role="tablist">{visibleTabs.map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} className={activeTab === tab.id ? "active" : ""} onClick={() => setActiveTab(tab.id)}>{tab.label}</button>)}</div><div className="sheet-meta"><span className="eyebrow">{active.label}</span><span>{active.hint}</span></div>{activeTab === "admin" && message && <p className="selection-message" role="status">{message}</p>}<div role="tabpanel">{activeTab === "archive" && <div className="sheet-archive"><HistoricalDraws albums={albums} /><LiveDraws entries={entries} member={member} onOpenProposal={goToProposal} onOpenReview={goToReview} /></div>}{activeTab === "selection" && <SelectionWorkspace entries={entries} currentDraw={currentPublishedDraw} member={member} reviews={reviews} focusedProposal={focusedProposal} focusedReview={focusedReview} savingId={savingId} onProposal={(payload) => void saveProposal(payload)} onReview={(payload) => void saveReview(payload)} />}{activeTab === "kouize" && <Kouize answers={kouizeAnswers} member={member} isAdmin={isAdmin} />}{activeTab === "admin" && isAdmin && <AdminDraws entries={entries} draws={draws} savingId={savingId} onCreate={(participants) => void createDraw(participants)} onDelete={(draw) => void deleteDraw(draw)} onPublish={(draw) => void publishDraw(draw)} onAssign={(entry, proposer, listener, confirmReset) => void assignEntry(entry, proposer, listener, confirmReset)} />}</div></section>;
}
