...
export function TableurBoard({ albums }: { albums: Album[] }) {
  ...
  const persistDraw = async ({ participants, drawType, globalProposer }: { participants: string[]; drawType: "standard" | "global"; globalProposer: string | null }) => { if (!configured || !isAdmin) return false; setSavingId("new-draw"); setMessage(""); const { error } = await getSupabaseBrowserClient().rpc("admin_create_club_draw", { p_participant_usernames: participants, p_avoid_repeated_pairs: true, p_draw_type: drawType, p_global_proposer_username: globalProposer }); setSavingId(null); if (error) { setMessage(error.message); return false; } setMessage(drawType === "global" ? "Ecoute globale creee : publie-la quand tu es pret." : "Nouveau brouillon cree : valide les duos avant de le publier."); await Promise.all([loadEntries(), loadDraws()]); return true; };
