...
function HistoricalReviewRow({ album, record, editable, saving, onSave }: { album: Album; record?: ArchivedReview; editable: boolean; saving: boolean; onSave: (payload: Omit<ArchivedReview, "is_modified" | "review_title">) => void }) {
...
function HistoricalPendingReviews({ albums, member }: { albums: Album[]; member: SignedMember | null }) {
  ...
  const loadRecords = useCallback(async () => {
    ...
    const [reviewResult, entryResult] = await Promise.all([
      getSupabaseBrowserClient().from("archived_album_reviews").select("album_id, review_title, review, rating, best_track, worst_track, is_modified"),
  ...
  const save = async ({ entryId, reviewTitle, review, rating, bestTrack, worstTrack }: ReviewPayload) => {
    ...
    const { error } = await getSupabaseBrowserClient().rpc("save_my_archived_album_review", { p_album_id: storageArchiveRecordId(entryId), p_review_title: reviewTitle?.trim() || null, p_review: review.trim(), p_rating: rating, p_best_track: bestTrack.trim() || null, p_worst_track: worstTrack.trim() || null, p_reset: false });
    setSavingId(null);
    if (error) setMessage(error.message);
    else { setMessage("Verdict enregistré dans le tirage."); await loadRecords(); }
  };
  const reset = async (entryId: string) => { if (!configured) return; setSavingId(entryId); setMessage(""); const { error } = await getSupabaseBrowserClient().rpc("save_my_archived_album_review", { p_album_id: storageArchiveRecordId(entryId), p_review_title: null, p_review: null, p_rating: null, p_best_track: null, p_worst_track: null, p_reset: true }); setSavingId(null); if (error) setMessage(error.message); else { setMessage("Avis reinitialise : l'album est a nouveau en attente."); await loadRecords(); } };
  return <section className="review-workspace historical-pending-workspace"><div className="review-workspace__heading"><div><p className="eyebrow">VERDICTS EN ATTENTE</p><h2>Les albums plus anciens que je dois <em>encore rendre.</em></h2><p>Chaque écoute sans note reste ici jusqu’à ce que tu enregistres ton verdict.</p></div><span className="review-counter">{pending.length} écoute{pending.length > 1 ? "s" : ""}</span></div>{message && <p className="selection-message" role="status">{message}</p>}<div className="review-queue">{pending.length ? pending.map((album) => {
    ...
    return <ReviewAssistantCard key={album.id} entry={entry} existing={(() => { const record = recordMap.get(storageArchiveRecordId(album.id)); return record?.is_modified && record.review && record.rating !== null ? { album_id: album.id, review_title: record.review_title, review: record.review, rating: record.rating, best_track: record.best_track, worst_track: record.worst_track } : undefined; })()} coverUrl={album.cover} saving={savingId === album.id} onSave={(payload) => void save(payload)} onReset={(entryId) => void reset(entryId)} />;
...
function AdminDraws({ albums = archivedAlbums, entries, draws, savingId, onCreate, onDelete, onValidate, onPublish }: { albums?: Album[]; entries: DrawEntry[]; draws: DrawMeta[]; savingId: string | null; onCreate: (config: { participants: string[]; drawType: "standard" | "global"; globalProposer: string | null }) => void; onDelete: (draw: number) => void; onValidate: (draw: number, assignments: Array<{ entry: DrawEntry; proposer: string; listener: string }>) => Promise<void>; onPublish: (draw: number) => void }) {
  ...
  const createDisabled = savingId === "new-draw" || participants.length < 2 || (drawType === "global" && !globalProposer);
  return <section className="draw-sheet admin-draw-sheet">
    <div className="draw-creator draw-creator--global-ready">
      ...
      <div className="draw-creator__setup">
        ...
        {drawType === "global" && <label className="global-draw-proposer"><span>Qui choisit l’album commun ?</span><MemberSelect value={globalProposer} options={globalOptions} label="Choisir le proposeur du tirage global" onChange={setGlobalProposer} /></label>}
      </div>
      <button type="button" className="button" disabled={createDisabled} onClick={() => onCreate({ participants, drawType, globalProposer: drawType === "global" ? globalProposer : null })}>{savingId === "new-draw" ? "Création…" : drawType === "global" ? "Créer l’écoute globale" : "Créer le tirage classique"}</button>
    </div>
    {draws.map((draw) => {
      const rows = (rowsByDraw.get(draw.draw_number) ?? []).sort((a, b) => a.position - b.position);
      ...
      return <section className={"draw-section" + (global ? " draw-section--global" : "")} key={draw.draw_number}>
        ...
        <div className="sheet-scroll"><table className="sheet-table admin-draw-table"><thead><tr><th>Ligne</th><th>Proposé par</th><th>Écouté par</th></tr></thead><tbody>{rows.map((entry) => {
          ...
          const proposedOptions = options.filter((member) => (member.username === current.proposer || !rows.some((other) => other.id !== entry.id && field(other).proposer === member.username)) && member.username !== current.listener);
          const listenerOptions = options.filter((member) => (member.username === current.listener || !rows.some((other) => other.id !== entry.id && field(other).listener === member.username)) && member.username !== current.proposer);
...
export function TableurBoard({ albums }: { albums: Album[] }) {
  ...
  const resetReview = async (entryId: string) => { if (!configured || !member) return; setSavingId(entryId); setMessage(""); try { const { data: deletedReview, error } = await getSupabaseBrowserClient().from("member_album_reviews").delete().eq("album_id", entryId).eq("member_id", member.id).select("id").maybeSingle(); if (error || !deletedReview?.id) throw error ?? new Error("Le verdict etait deja absent."); await Promise.all([loadEntries(), loadReviews(), loadPublicReviews()]); setMessage("Verdict réinitialisé : la ligne est de nouveau en attente."); returnToDraw(entryId); } catch (error) { setMessage(error instanceof Error ? error.message : "Le verdict n’a pas pu être réinitialisé."); } finally { setSavingId(null); } };
  const createDraw = async ({ participants, drawType, globalProposer }: { participants: string[]; drawType: "standard" | "global"; globalProposer: string | null }) => { if (!configured || !isAdmin) return; setSavingId("new-draw"); setMessage(""); const { error } = await getSupabaseBrowserClient().rpc("admin_create_club_draw", { p_participant_usernames: participants, p_avoid_repeated_pairs: true, p_draw_type: drawType, p_global_proposer_username: globalProposer }); setSavingId(null); if (error) setMessage(error.message); else { setMessage(drawType === "global" ? "Écoute globale créée : publie-la quand tu es prêt." : "Nouveau brouillon créé : valide les duos avant de le publier."); await Promise.all([loadEntries(), loadDraws()]); } };
