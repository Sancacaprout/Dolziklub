...
export function MemeGallery({ memes }: { memes: Meme[] }) {
  ...
  const displayedMemes = useMemo<DisplayedMeme[]>(() => {
    ...
    const remote = uploadedMemes.map((post) => {
      ...
      return { id: `upload-${post.id}`, title: "Mème du Ziklub", src: data.publicUrl, alt: post.caption || "Mème ajouté par un membre du Ziklub", caption: post.caption };
    });
    return [...remote, ...memes];
