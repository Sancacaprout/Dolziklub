...
export function MemberStatsCards({ username, base }: { username: string; base: { proposed: number; listened: number; givenAverage: number | null; givenCount: number; receivedAverage: number | null; receivedCount: number } }) {
  ...
  return <div className="stat-cards"><div style={cardStyle(colors.proposals)}><b>{proposed}</b><span>propositions</span></div><div style={cardStyle(colors.listens)}><b>{listened}</b><span>écoutes</span></div><div style={cardStyle(colors.given_average)}><b>{format(givenAverage)}</b><span className="stat-cards__label">Note moyenne attribuée</span></div><div style={cardStyle(colors.given_average)}><b>{format(receivedAverage)}</b><span className="stat-cards__label">Note moyenne obtenue</span></div></div>;
}
