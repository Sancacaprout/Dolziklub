...
export function MemberStatsCards({ username, base }: { username: string; base: { proposed: number; listened: number; givenAverage: number | null; givenCount: number; receivedAverage: number | null; receivedCount: number } }) {
  ...
  useEffect(() => { if (!configured) return; const supabase = getSupabaseBrowserClient(); void Promise.all([supabase.from("member_public_profiles").select("stat_colors").eq("username", username).maybeSingle(), supabase.rpc("get_public_member_draw_metrics", { p_username: username })]).then(([profile, result]) => { const source = profile.data?.stat_colors && typeof profile.data.stat_colors === "object" ? profile.data.stat_colors as Record<string, unknown> : {}; const unified = color(source.unified ?? source.proposals ?? source.listens ?? source.given_average, defaults.proposals); setColors({ proposals: unified, listens: unified, given_average: unified }); if (!result.error && result.data?.[0]) setMetrics(result.data[0] as Metrics); }); }, [configured, username]);
  ...
  return <div className="stat-cards"><div style={{ backgroundColor: colors.proposals }}><b>{proposed}</b><span>propositions</span></div><div style={{ backgroundColor: colors.listens }}><b>{listened}</b><span>écoutes</span></div><div style={{ backgroundColor: colors.given_average }}><b>{format(givenAverage)}</b><span className="stat-cards__label">Note moyenne attribuée</span></div><div style={{ backgroundColor: colors.given_average }}><b>{format(receivedAverage)}</b><span className="stat-cards__label">Note moyenne obtenue</span></div></div>;
}
