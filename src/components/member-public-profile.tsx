...
export function MemberPublicProfile({ displayName, username, role }: { displayName: string; username: string | null; role: "admin" | "member" }) {
  ...
  return <><section className="member-profile" data-profile-part="header"><div className="member-profile__initial member-profile__avatar" data-profile-part="avatar">{avatar ? <Image src={avatar} alt={`Affiche de ${displayName}`} width={220} height={220} /> : displayName[0]}</div><div><p className="eyebrow">MEMBRE DU DOL ZIKLUB</p><h1>{displayName}</h1><p>{role === "admin" ? "Membre (admin)" : "Membre"} · @{username}</p></div></section>{(profile?.bio || answers.length > 0 || profile?.kouize.note) && <section className="member-kouize" style={kouizeStyle} data-profile-part="quiz"><div><p className="eyebrow">KOUIZE DE {displayName.toUpperCase()}</p><h2>Ce qui lui<br /><em>fait tendre l’oreille.</em></h2>{profile?.bio && <p className="member-kouize__bio">{profile.bio}</p>}</div><div className="member-kouize__answers">{answers.map((answer) => <div key={answer.label}><span>{answer.label}</span><b>{answer.value}</b></div>)}{profile?.kouize.note && <p>“{profile.kouize.note}”</p>}</div></section>}</>;
}
