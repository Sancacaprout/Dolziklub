...
export function MemberBonusReviews({ username, albums = [] }: { username: string | null; albums?: Album[] }) {
  ...
  return <section className="member-archive member-bonus-reviews" data-profile-part="bonus" data-profile-section="bonus"><div className="member-archive__heading"><div><p className="eyebrow">{"\u00c9COUTES BONUS"}</p><h2>{"Albums \u00e9cout\u00e9s en bonus."}</h2></div><small>Hors statistiques du club</small></div><div className="member-bonus-reviews__list">{ordered.map((review) => {
    ...
    return <article key={`${review.entry_id ?? review.archive_album_id}:${review.member_username}`} className="member-bonus-review" data-profile-part="bonus-card"><div className="member-bonus-review__cover" data-profile-part="cover">{coverUrl ? <img src={coverUrl} alt={`Pochette de ${title}`} /> : <span aria-hidden="true">DOL<br />ZIKLUB</span>}</div><div className="member-bonus-review__main" data-profile-part="copy"><span className="eyebrow">{review.archive_number == null ? `TIRAGE ${String(review.draw_number ?? 0).padStart(2, "0")}` : `ARCHIVE #${review.archive_number} - TIRAGE ${String(review.draw_number ?? 0).padStart(2, "0")}`}</span><h3>{title}</h3><p>{artist}</p><div className="member-bonus-review__verdict"><ReviewPreview title={review.review_title} review={review.review} /></div></div><RatingDisplay rating={review.rating} /></article>;
  })}</div></section>;
}