...
type BadgeShelf = { id: string; label: string; badges: MemberBadge[] };
...
export function BadgeCollection() {
  ...
  const shelves = useMemo<BadgeShelf[]>(() => {
    const badges = collection?.badges ?? [];
    const regularShelves = stateOrder.map((state) => ({
      id: state,
      label: stateLabels[state],
      badges: badges.filter((badge) => !badge.secret && badge.state === state),
    })).filter((shelf) => shelf.badges.length > 0);
    const secrets = badges.filter((badge) => badge.secret);
    return secrets.length ? [...regularShelves, { id: "secret", label: "Badges secrets", badges: secrets }] : regularShelves;
  }, [collection]);
  ...
  const claim = async (key: BadgeKey) => {
    ...
    if (error) setMessage("Ce badge n’a pas pu être réclamé.");
      ...
      setMessage("Badge ajouté à ta collection.");
  ...
  const equip = async (slot: 1 | 2 | 3, key: BadgeKey | null) => {
    ...
    if (error) setMessage("Cet emplacement n’a pas pu être modifié.");
      ...
      setCollection(data as BadgeCollectionPayload);
      setMessage(key ? `Badge équipé dans l’emplacement ${slot}.` : `Emplacement ${slot} libéré.`);
    }
    setBusy(null);
  };
  ...
  return <section className="page badge-collection" aria-labelledby="my-badges-heading">
    <header className="badge-collection__header">
      <div><p className="eyebrow">COLLECTION PERSONNELLE</p><h2 id="my-badges-heading">Mes badges</h2></div>
      <p>{collection.badges.filter((badge) => badge.state !== "locked").length} / 29 débloqués</p>
    </header>
    <section className="badge-slots" aria-labelledby="badge-slots-heading">
      <div><p className="eyebrow">PROFIL PUBLIC</p><h3 id="badge-slots-heading">Mes trois badges exposés</h3></div>
      <div className="badge-slots__grid">{([1, 2, 3] as const).map((slot) => {
        const badge = equipped.find((item) => item.slot === slot);
        return <article className="badge-slot" key={slot}>
          <span>EMPLACEMENT {slot}</span>
          {badge ? <><BadgeArt imagePath={badge.imagePath} name={badge.name} rarity={badge.rarity} size={88} /><b>{badge.name}</b><button type="button" onClick={() => void equip(slot, null)} disabled={busy === `slot:${slot}`}>Retirer</button></> : <p>Libre</p>}
        </article>;
      })}</div>
    </section>
    {shelves.map((shelf) => <section className={`badge-shelf badge-shelf--${shelf.id}`} key={shelf.id} aria-labelledby={`badges-${shelf.id}`}>
      <header><h3 id={`badges-${shelf.id}`}>{shelf.label}</h3><span>{shelf.badges.length}</span></header>
      <div className="badge-grid">{shelf.badges.map((badge) => {
        const progress = badge.progress;
        const ratio = progress ? Math.min(100, Math.round((progress.current / Math.max(progress.target, 1)) * 100)) : 0;
        return <article className={`badge-card badge-card--${badge.rarity} is-${badge.state}`} key={badge.key}>
          <BadgeArt imagePath={badge.imagePath} name={badge.name} rarity={badge.rarity} locked={badge.state === "locked"} />
          <div className="badge-card__copy"><span>{badge.category}</span><h4>{badge.name}</h4><p>{badge.description}</p></div>
