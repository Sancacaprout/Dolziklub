...
type Draw = { draw_number: number; status: "draft" | "published" | "locked"; created_at: string; published_at: string | null; locked_at: string | null };
...
export async function getHomeActivity(): Promise<HomeActivity[]> {
  ...
  try {
    ...
    const [entriesResult, reviewsResult, memesResult, drawsResult] = await Promise.all([
      ...
      supabase.from("club_draws").select("draw_number, status, created_at, published_at, locked_at").order("draw_number", { ascending: false }).limit(1),
    ]);
    ...
    if (latestDraw) {
      const namedAlbums = entries.filter((entry) => entry.draw_number === latestDraw.draw_number).length;
