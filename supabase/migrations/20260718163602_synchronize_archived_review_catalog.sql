-- Archived verdicts are part of the shared catalogue. A reset is also an override:
-- it must replace a former static verdict with the pending state everywhere.
...
begin
  ...
  select profile.username into current_username
  ...
  set
    review_title = case when p_reset then null else coalesce(nullif(btrim(p_review_title), ''), archive_review.review_title) end,
